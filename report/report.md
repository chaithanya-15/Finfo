...
strategies (Section 4.2): restricting candidates to the company named in the question, fusing the
...
words (Section 5.2). Precision@k divides by k rather than by the number of passages returned, so
...
(Section 5.2). A 0.019 gap is small enough that the difference in indexed
...
84 questions with a numeric gold answer (Section 2.8).
